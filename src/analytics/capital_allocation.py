...
def classify_pattern(cfo, cfi, cff, borrowing_change):
    """Classify pattern."""
...
def main():
    ...
    print(f"Balance Sheet: {len(balancesheet)} rows")
    ...
    balancesheet["year"] = pd.to_numeric(balancesheet["year"], errors="coerce")
    ...
    for column in [
        ...
        cashflow[column] = pd.to_numeric(cashflow[column], errors="coerce")

    balancesheet["borrowings"] = pd.to_numeric(
        balancesheet["borrowings"], errors="coerce"
    )
    ...
    for company_id in company_ids:
        ...
        for _, row in cf.iterrows():
            ...
            year_bs = bs[bs["year"] == year]

            current_borrowings = np.nan

            if not year_bs.empty:
                current_borrowings = year_bs.iloc[-1]["borrowings"]

            borrowing_change = np.nan

            if not pd.isna(previous_borrowings) and not pd.isna(current_borrowings):
                borrowing_change = current_borrowings - previous_borrowings

            pattern = classify_pattern(cfo, cfi, cff, borrowing_change)
            ...
            if not pd.isna(current_borrowings):
                previous_borrowings = current_borrowings
    ...
    if history.empty:
        raise RuntimeError("No capital allocation records were generated.")
    ...
    history.to_csv(PATTERN_PATH, index=False)
    ...
    for company_id, group in history.groupby("company_id"):
        ...
        valid = group[group["capital_allocation_pattern"] != "Insufficient Data"].copy()
    ...
    if not changes.empty:
        ...
        changes = changes.sort_values("company_id").reset_index(drop=True)
    ...
    company_count = history["company_id"].nunique()

    years_per_company = history.groupby("company_id")["year"].nunique()

    print("\n" + "=" * 70)
    print("CAPITAL ALLOCATION OUTPUT")
    print("=" * 70)

    print(f"Companies represented : " f"{company_count}")

    print(f"92 companies complete : " f"{company_count == 92}")

    print(f"Total yearly records  : " f"{len(history)}")

    print(f"Minimum years/company : " f"{years_per_company.min()}")

    print(f"Maximum years/company : " f"{years_per_company.max()}")

    print("\nPattern distribution across all years:")

    print(history["capital_allocation_pattern"].value_counts().to_string())

    print("\nEight defined patterns:")

    for pattern in expected_patterns:

        count = (history["capital_allocation_pattern"] == pattern).sum()
    ...
    latest_rows = (
        history.sort_values("year").groupby("company_id", as_index=False).tail(1)
    )

    print(latest_rows["capital_allocation_pattern"].value_counts().to_string())

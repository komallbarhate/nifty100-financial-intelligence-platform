...
def find_column(columns, aliases):
    """Find column."""
    normalised_columns = {normalise(column): column for column in columns}
...
def main():
    ...
    df = pd.read_excel(ANALYSIS_PATH, sheet_name=0, header=1)
    ...
    if company_col is None:
        raise ValueError(
            "Could not identify the company ID column " "in analysis.xlsx."
        )
    ...
    for metric, aliases in TARGETS.items():
        ...
        column = find_column(df.columns, aliases)
    ...
    for metric in TARGETS:
        print(f"  {metric}: " f"{target_columns.get(metric)}")
    ...
    for _, row in df.iterrows():
        ...
        company_id = str(row[company_col]).strip()
    ...
    if not ratios.empty:
        ...
        latest_ratios = ratios.groupby("company_id", as_index=False).tail(1)
        ...
        for metric, ratio_column in ratio_map.items():
            ...
            latest_ratios[ratio_column] = pd.to_numeric(
                latest_ratios[ratio_column], errors="coerce"
            )

            ratio_lookup = latest_ratios.set_index("company_id")[ratio_column].to_dict()

            metric_rows = parsed[parsed["metric_type"] == metric]

            for _, parsed_row in metric_rows.iterrows():

                if parsed_row["period_years"] != 5:
                    continue

                company_id = parsed_row["company_id"]

                computed = ratio_lookup.get(company_id)
                ...
                parsed_value = parsed_row["value_pct"]

                divergence = abs(parsed_value - computed) / abs(computed) * 100
    ...
    if not validation_df.empty:
        ...
        manual_reviews = int(validation_df["manual_review"].sum())

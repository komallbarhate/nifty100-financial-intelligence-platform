...
class CompositeScorer:
    ...
    def calculate_fcf_cagr(self):
        ...
        for company_id, group in self.data.groupby(
            "company_id"
        ):
...
def main():
    ...
    print(
        f"Score minimum: "
        f"{result['sprint3_composite_score'].min():.2f}"
    )

    print(
        f"Score maximum: "
        f"{result['sprint3_composite_score'].max():.2f}"
    )

    print(
        f"Score average: "
        f"{result['sprint3_composite_score'].mean():.2f}"
    )
